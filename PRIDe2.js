...
client.on('message', msg => {
  if (msg.content !== '') {
    console.log('[CHAT]' + msg.author.tag + ' > ' + msg.content);
  }
})
...
if(msg.startswith == "Api") {
  message.channel.send("Phoenix and Hasan's API , Phoenix's Asistant  , Blaze")
}
});
...
const commands = {
	'play': (msg) => {
		...
		(function play(song) {
			...
			msg.channel.sendMessage(`Playing: **${song.title}** as requested by: DJ **${song.requester}**`);
	...
	'reboot': (msg) => {
      ...
      msg.reply("Restarting Please Wait") //Requires a node module like Forever to work.
    process.exit();
    }
  }
};
...
client.on('message', message => {
    if (message.author.id === "338332694725263361") {
    if (message.content.startsWith(prefix + "Connect.Phoenix.Sys")) {
        message.guild.channels.find('name', 'bot-logs').sendMessage("**[PHOENIX_PROCESS]** Accessing Phoenix Systems...")    
        message.guild.channels.find('name', 'bot-logs').sendMessage("Password Enter:")
        message.guild.channels.find('name', 'bot-logs').sendMessage("Connected To Phoenix Systems And Security")
    }
    }
});
//////////////////////////////
